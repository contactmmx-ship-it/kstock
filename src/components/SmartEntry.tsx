...
export default function SmartEntry({ onSubmit, disabled }: SmartEntryProps) {
  ...
  const { isListening, transcript, supported, startListening, clearTranscript } = useVoiceInput();
      ...
      <div className="relative">
        <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-2xl px-4 py-3 shadow-sm focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20 transition-all">
          ...
          <input
            ...
            placeholder='Try: "Paid 500 to Mr Raj in cash for office rent at 2pm"'
          ...
          {supported && (
            <button
              ...
              className={`p-2 rounded-xl transition-all ${isListening ? 'bg-red-100 text-red-600 animate-pulse' : 'hover:bg-slate-100 text-slate-400 hover:text-slate-600'}`}
              title="Voice input"
            >
